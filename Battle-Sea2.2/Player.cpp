...
void Pole::printPole(Player& player) {//друк пол€ п≥д час розтановки карабл≥в
...
void Pole::printPole(int x, int y, char ch)//друк пол€ п≥д час руху по ньому
{
	char arr[11]{ '-','A', 'B', 'C','D','E','F','G','H','I','J' };
	if (getAmount() == true) {}
	else {
...
bool Pole::getAmount()//метод перев≥рки к≥лькост≥ поставлених палуб кораблик≥в
{
	int value = 0;
	for (int i = 0; i < n; i++)
	{
...
bool Pole::getShot(Player& playe, Pole& pol)//метод перев≥рки чи вистр≥л попав у палубу карабл€
{
	int x = playe.getX();
	int y = playe.getY();
	char ch = playe.getCh();
...
bool Pole::getWinner()//метод перев≥рки хто виграв
{
	int sum = 0;
	int sum2 = 0;
	for (int i = 0; i < n; i++)
