...
class Menu
{private:
	enum Color { Black, Blue, Green, Cyan, Red, Magenta, Brown, LightGray, DarkGray, LightBlue = 9, LightGreen, LightCyan, LightRed, LightMagenta, Yellow, White,Purple	};//перечислення )кольори 
	HANDLE hStdOut = GetStdHandle(STD_OUTPUT_HANDLE); //Дескриптор активного вікна
public:
	void instruction(void);//метод виводу правил гри
	...
	{
		setColor(White, Cyan); //встановлюю колір текста 
